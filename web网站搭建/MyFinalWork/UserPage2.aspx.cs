...
{
    public partial class UserPage2 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    string query = "SELECT 失物名称,失主,发布用户,丢失时间,丢失地点,联系方法 FROM Lost"; // 查询获取失物名称、失主、发布用户这三列数据
                    ...
                    try
                    {
                        ...
                        if (reader.HasRows) // 判断是否有数据行
                        {
                            reader.Read();
                            // 将失物名称赋值给Label2
                            Label2.Text = reader["失物名称"].ToString();
                            // 将失主名称赋值给Label4
                            Label4.Text = reader["失主"].ToString();
                            // 将发布用户名称赋值给Label6
                            Label6.Text = reader["发布用户"].ToString();
                            Label8.Text = reader["丢失时间"].ToString();
                            Label10.Text = reader["丢失地点"].ToString();
                            Label30.Text = reader["联系方法"].ToString();
                        }
                        else
                        {
                            Label2.Text = "null";
                        ...
                        {
                            reader.Read();
                            Label11.Text = reader["失物名称"].ToString();
                            Label13.Text = reader["失主"].ToString();
                            Label15.Text = reader["发布用户"].ToString();
                            Label17.Text = reader["丢失时间"].ToString();
                            Label19.Text = reader["丢失地点"].ToString();
                            Label32.Text = reader["联系方法"].ToString();
                        }
                        else
                        {
                            Label11.Text = "null";
                        ...
                        {
                            reader.Read();
                            Label20.Text = reader["失物名称"].ToString();
                            Label22.Text = reader["失主"].ToString();
                            Label24.Text = reader["发布用户"].ToString();
                            Label26.Text = reader["丢失时间"].ToString();
                            Label28.Text = reader["丢失地点"].ToString();
                            Label34.Text = reader["联系方法"].ToString();
                        }
                        else
                        {
                            Label11.Text = "null";
        ...
        {
            string strCommand;
            strCommand = $"select * from Lost where 失物名称 like '%{searchname}%'";
        ...
        {
            Response.Redirect("PubLost.aspx");
        }
        ...
        protected void Button7_Click(object sender, EventArgs e)
        {
            ...
            // 构建查询Found表所有数据的语句
            string allDataCommand = "select * from Lost";
